...
function computeFilterId(mask, selector, domains, parentDomains, style) {
    ...
    if (domains !== undefined) {
        hash = (hash * utils_js_1.HASH_INTERNAL_MULT) ^ HASH_DOMAINS_MARKER_LOW;
        hash = domains.updateId(hash);
    }
    if (parentDomains !== undefined) {
        hash = (hash * utils_js_1.HASH_INTERNAL_MULT) ^ HASH_DOMAINS_MARKER_HIGH;
...
class CosmeticFilter {
    ...
    static parse(line, debug = false) {
        ...
        if (sharpIndex > 0) {
            ...
            if (domainEntries.length !== 0) {
                domains = domains_js_1.Domains.parse(domainEntries.join(','), {
                    debug,
                });
            }
            if (parentDomainEntries.length !== 0) {
                parentDomains = domains_js_1.Domains.parse(parentDomainEntries.join(','), {
                    debug,
                });
            }
        }
        if (line.length - suffixStartIndex >= 8 &&
            line.endsWith(')') &&
        ...
        if (line.charCodeAt(suffixStartIndex) === 94 /* '^' */) {
            ...
            if (selectorType === adblocker_extended_selectors_1.SelectorType.Extended) {
                ...
                selector.slice(0, (0, adblocker_extended_selectors_1.indexOfPseudoDirective)(selector)).trim().length === 0) {
                    return null;
                }
                mask = (0, utils_js_1.setBit)(mask, 64 /* COSMETICS_MASK.extended */);
            }
        ...
        if (parentDomains !== undefined && (0, utils_js_1.getBit)(mask, 2 /* COSMETICS_MASK.scriptInject */) === false) {
            return null;
        }
        // Extended selectors should always be specific to some domain.
        if (domains === undefined && (0, utils_js_1.getBit)(mask, 64 /* COSMETICS_MASK.extended */) === true) {
            return null;
        }
        if (selector !== undefined) {
            ...
            if ((0, utils_js_1.getBit)(mask, 2 /* COSMETICS_MASK.scriptInject */) === false &&
                (0, utils_js_1.getBit)(mask, 64 /* COSMETICS_MASK.extended */) === false &&
    ...
    static deserialize(buffer) {
        ...
        return new CosmeticFilter({
            ...
            parentDomains: (optionalParts & 8) === 8 ? domains_js_1.Domains.deserialize(buffer) : undefined,
        });
    }
    // Mandatory fields
    mask;
    ...
    match(hostname, domain, ancestors) {
        ...
        if (this.domains !== undefined &&
            ...
            this.domains.match((0, request_js_1.getHostnameHashesFromLabelsBackward)(hostname, domain), (0, request_js_1.getEntityHashesFromLabelsBackward)(hostname, domain))) {
            return true;
        }
    ...
    getASTComponents() {
        ...
        return (0, adblocker_extended_selectors_1.destructAST)(ast);
    }
    getExtendedSelector() {
        return (0, html_filtering_js_1.extractHTMLSelectorFromRule)(this.selector);
    }
    isExtended() {
        return (0, utils_js_1.getBit)(this.mask, 64 /* COSMETICS_MASK.extended */);
    }
    /**
    ...
    isGenericHide() {
        return (this?.domains?.hostnames === undefined &&
