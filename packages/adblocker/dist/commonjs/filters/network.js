...
const REQUEST_TYPE_TO_MASK = {
    ...
    fedcm: 512 /* NETWORK_FILTER_MASK.fromScript */,
    ...
    json: 512 /* NETWORK_FILTER_MASK.fromScript */,
...
function computeFilterId(mask, filter, hostname, domains, denyallow, optionValue, isCaseSensitive) {
    ...
    if (isCaseSensitive !== undefined) {
        hash = (hash * utils_js_1.HASH_INTERNAL_MULT) ^ (1 << +isCaseSensitive);
    }
    return hash >>> 0;
}
/**
...
class NetworkFilter {
            ...
            for (const rawOption of getFilterOptions(line, optionsIndex + 1, line.length)) {
                ...
                switch (option) {
                    ...
                    case 'redirect': {
                        ...
                        mask = (0, utils_js_1.setBit)(mask, 1073741824 /* NETWORK_FILTER_MASK.isRedirect */);
                    ...
                    case 'content':
                        if (negation ||
                            (value.length === 0
                                ? (0, utils_js_1.getBit)(mask, 134217728 /* NETWORK_FILTER_MASK.isException */) === false
                                : replaceOptionValueToRegexp(value) === null) ||
                            optionValue !== undefined) {
                            return null;
                        }
                        mask = (0, utils_js_1.setBit)(mask, 65536 /* NETWORK_FILTER_MASK.isReplace */);
                        optionValue = value;
                        break;
                    case 'removeparam':
                        // TODO: Support regex
                        ...
                        mask = (0, utils_js_1.setBit)(mask, -2147483648 /* NETWORK_FILTER_MASK.isRemoveParam */);
                        optionValue = value;
                        break;
                    default: {
        ...
        if (filterIndexEnd - filterIndexStart >= 2 &&
            ...
            if (filterIndexEnd - filterIndexStart > 0) {
                ...
                if ((0, utils_js_1.getBit)(mask, 8388608 /* NETWORK_FILTER_MASK.isRegex */) === false &&
                    ...
                    mask = setNetworkMask(mask, 8388608 /* NETWORK_FILTER_MASK.isRegex */, true);
                }
                else {
    ...
    static deserialize(buffer) {
        ...
        return new NetworkFilter({
            ...
            mask,
            // Optional parts
            filter: (0, utils_js_1.getBit)(optionalParts, 1 /* NETWORK_FILTER_OPTIONAL_PARTS_MASK.hasFilter */)
                ? isUnicode
                    ? buffer.getUTF8()
                    : buffer.getNetworkFilter()
                : undefined,
            hostname: (0, utils_js_1.getBit)(optionalParts, 2 /* NETWORK_FILTER_OPTIONAL_PARTS_MASK.hasHostname */)
                ? buffer.getNetworkHostname()
                : undefined,
            domains: (0, utils_js_1.getBit)(optionalParts, 4 /* NETWORK_FILTER_OPTIONAL_PARTS_MASK.hasDomains */)
                ? domains_js_1.Domains.deserialize(buffer)
                : undefined,
            rawLine: (0, utils_js_1.getBit)(optionalParts, 8 /* NETWORK_FILTER_OPTIONAL_PARTS_MASK.hasRawLine */)
                ? buffer.getRawNetwork()
                : undefined,
            denyallow: (0, utils_js_1.getBit)(optionalParts, 16 /* NETWORK_FILTER_OPTIONAL_PARTS_MASK.hasDenyallow */)
                ? domains_js_1.Domains.deserialize(buffer)
                : undefined,
            optionValue: (0, utils_js_1.getBit)(optionalParts, 32 /* NETWORK_FILTER_OPTIONAL_PARTS_MASK.hasOptionValue */)
                ? (0, utils_js_1.getBit)(mask, 262144 /* NETWORK_FILTER_MASK.isCSP */)
                    ? buffer.getNetworkCSP()
                    : (0, utils_js_1.getBit)(mask, 1073741824 /* NETWORK_FILTER_MASK.isRedirect */)
                        ? buffer.getNetworkRedirect()
                        : buffer.getUTF8()
                : undefined,
            regex: undefined,
            isCaseSensitive: (0, utils_js_1.getBit)(optionalParts, 64 /* NETWORK_FILTER_OPTIONAL_PARTS_MASK.isCaseSensitive */),
        });
    }
    filter;
    hostname;
    ...
    serialize(buffer) {
        ...
        if (this.filter !== undefined) {
            optionalParts = (0, utils_js_1.setBit)(optionalParts, 1 /* NETWORK_FILTER_OPTIONAL_PARTS_MASK.hasFilter */);
            if (this.isUnicode()) {
        ...
        if (this.hostname !== undefined) {
            optionalParts = (0, utils_js_1.setBit)(optionalParts, 2 /* NETWORK_FILTER_OPTIONAL_PARTS_MASK.hasHostname */);
            buffer.pushNetworkHostname(this.hostname);
        }
        if (this.domains !== undefined) {
            optionalParts = (0, utils_js_1.setBit)(optionalParts, 4 /* NETWORK_FILTER_OPTIONAL_PARTS_MASK.hasDomains */);
            this.domains.serialize(buffer);
        }
        if (this.rawLine !== undefined) {
            optionalParts = (0, utils_js_1.setBit)(optionalParts, 8 /* NETWORK_FILTER_OPTIONAL_PARTS_MASK.hasRawLine */);
            buffer.pushRawNetwork(this.rawLine);
        }
        if (this.denyallow !== undefined) {
            optionalParts = (0, utils_js_1.setBit)(optionalParts, 16 /* NETWORK_FILTER_OPTIONAL_PARTS_MASK.hasDenyallow */);
            this.denyallow.serialize(buffer);
        }
        if (this.optionValue !== undefined) {
            optionalParts = (0, utils_js_1.setBit)(optionalParts, 32 /* NETWORK_FILTER_OPTIONAL_PARTS_MASK.hasOptionValue */);
            if (this.isCSP()) {
        ...
        if (this.isCaseSensitive) {
            optionalParts = (0, utils_js_1.setBit)(optionalParts, 64 /* NETWORK_FILTER_OPTIONAL_PARTS_MASK.isCaseSensitive */);
        }
    ...
    getIdWithoutBadFilter() {
        ...
        return computeFilterId(this.mask & ~131072 /* NETWORK_FILTER_MASK.isBadFilter */, this.filter, this.hostname, this.domains, this.denyallow, this.optionValue, this.isCaseSensitive);
    }
    getId() {
        if (this.id === undefined) {
            this.id = computeFilterId(this.mask, this.filter, this.hostname, this.domains, this.denyallow, this.optionValue, this.isCaseSensitive);
        }
        return this.id;
    }
    hasFilter() {
    ...
    isRedirect() {
        return (0, utils_js_1.getBit)(this.getMask(), 1073741824 /* NETWORK_FILTER_MASK.isRedirect */);
    }
    isRedirectRule() {
    ...
    isReplace() {
        return (0, utils_js_1.getBit)(this.getMask(), 65536 /* NETWORK_FILTER_MASK.isReplace */);
    }
    isRemoveParam() {
        return (0, utils_js_1.getBit)(this.getMask(), -2147483648 /* NETWORK_FILTER_MASK.isRemoveParam */);
    }
    ...
    getTokens() {
        ...
        if (this.isFullRegex() === false) {
            if (this.filter !== undefined) {
                ...
                (0, utils_js_1.tokenizeWithWildcardsInPlace)(this.isRegex() ? this.filter.toLowerCase() : this.filter, skipFirstToken, skipLastToken, tokens_buffer_js_1.TOKENS_BUFFER);
            }
            ...
            (0, utils_js_1.tokenizeRegexInPlace)(this.filter.toLowerCase(), tokens_buffer_js_1.TOKENS_BUFFER);
        }
