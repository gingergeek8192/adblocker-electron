...
class Metadata {
    ...
    fromId(id) {
        ...
        for (const pattern of this.patterns.get(id)) {
            results.push({
                pattern,
                category: this.categories.get((0, categories_js_1.getKey)({ key: pattern.category }))?.[0],
                organization: pattern.organization !== null
                    ? this.organizations.get((0, organizations_js_1.getKey)({ key: pattern.organization }))?.[0]
                    : null,
            });
        }
        return results;
