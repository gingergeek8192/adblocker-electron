...
export default class NetworkFilter {
            ...
            for (const rawOption of getFilterOptions(line, optionsIndex + 1, line.length)) {
                ...
                switch (option) {
                    ...
                    case 'redirect': {
                        ...
                        mask = setBit(mask, 1073741824 /* NETWORK_FILTER_MASK.isRedirect */);
                    ...
                    case 'content':
                        if (negation ||
                            (value.length === 0
                                ? getBit(mask, 134217728 /* NETWORK_FILTER_MASK.isException */) === false
                                : replaceOptionValueToRegexp(value) === null) ||
                            optionValue !== undefined) {
                            return null;
                        }
                        mask = setBit(mask, 65536 /* NETWORK_FILTER_MASK.isReplace */);
                        optionValue = value;
                        break;
                    case 'removeparam':
                        // TODO: Support regex
                        ...
                        mask = setBit(mask, -2147483648 /* NETWORK_FILTER_MASK.isRemoveParam */);
                        optionValue = value;
                        break;
                    default: {
    ...
    static deserialize(buffer) {
        ...
        return new NetworkFilter({
            ...
            mask,
            // Optional parts
            filter: getBit(optionalParts, 1 /* NETWORK_FILTER_OPTIONAL_PARTS_MASK.hasFilter */)
                ? isUnicode
                    ? buffer.getUTF8()
                    : buffer.getNetworkFilter()
                : undefined,
            hostname: getBit(optionalParts, 2 /* NETWORK_FILTER_OPTIONAL_PARTS_MASK.hasHostname */)
                ? buffer.getNetworkHostname()
                : undefined,
            domains: getBit(optionalParts, 4 /* NETWORK_FILTER_OPTIONAL_PARTS_MASK.hasDomains */)
                ? Domains.deserialize(buffer)
                : undefined,
            rawLine: getBit(optionalParts, 8 /* NETWORK_FILTER_OPTIONAL_PARTS_MASK.hasRawLine */)
                ? buffer.getRawNetwork()
                : undefined,
            denyallow: getBit(optionalParts, 16 /* NETWORK_FILTER_OPTIONAL_PARTS_MASK.hasDenyallow */)
                ? Domains.deserialize(buffer)
                : undefined,
            optionValue: getBit(optionalParts, 32 /* NETWORK_FILTER_OPTIONAL_PARTS_MASK.hasOptionValue */)
                ? getBit(mask, 262144 /* NETWORK_FILTER_MASK.isCSP */)
                    ? buffer.getNetworkCSP()
                    : getBit(mask, 1073741824 /* NETWORK_FILTER_MASK.isRedirect */)
                        ? buffer.getNetworkRedirect()
                        : buffer.getUTF8()
                : undefined,
            regex: undefined,
            isCaseSensitive: getBit(optionalParts, 64 /* NETWORK_FILTER_OPTIONAL_PARTS_MASK.isCaseSensitive */),
        });
    }
    filter;
    hostname;
    ...
    serialize(buffer) {
        ...
        if (this.filter !== undefined) {
            optionalParts = setBit(optionalParts, 1 /* NETWORK_FILTER_OPTIONAL_PARTS_MASK.hasFilter */);
            if (this.isUnicode()) {
        ...
        if (this.hostname !== undefined) {
            optionalParts = setBit(optionalParts, 2 /* NETWORK_FILTER_OPTIONAL_PARTS_MASK.hasHostname */);
            buffer.pushNetworkHostname(this.hostname);
        }
        if (this.domains !== undefined) {
            optionalParts = setBit(optionalParts, 4 /* NETWORK_FILTER_OPTIONAL_PARTS_MASK.hasDomains */);
            this.domains.serialize(buffer);
        }
        if (this.rawLine !== undefined) {
            optionalParts = setBit(optionalParts, 8 /* NETWORK_FILTER_OPTIONAL_PARTS_MASK.hasRawLine */);
            buffer.pushRawNetwork(this.rawLine);
        }
        if (this.denyallow !== undefined) {
            optionalParts = setBit(optionalParts, 16 /* NETWORK_FILTER_OPTIONAL_PARTS_MASK.hasDenyallow */);
            this.denyallow.serialize(buffer);
        }
        if (this.optionValue !== undefined) {
            optionalParts = setBit(optionalParts, 32 /* NETWORK_FILTER_OPTIONAL_PARTS_MASK.hasOptionValue */);
            if (this.isCSP()) {
        ...
        if (this.isCaseSensitive) {
            optionalParts = setBit(optionalParts, 64 /* NETWORK_FILTER_OPTIONAL_PARTS_MASK.isCaseSensitive */);
        }
    ...
    isRedirect() {
        return getBit(this.getMask(), 1073741824 /* NETWORK_FILTER_MASK.isRedirect */);
    }
    isRedirectRule() {
    ...
    isReplace() {
        return getBit(this.getMask(), 65536 /* NETWORK_FILTER_MASK.isReplace */);
    }
    isRemoveParam() {
        return getBit(this.getMask(), -2147483648 /* NETWORK_FILTER_MASK.isRemoveParam */);
    }
