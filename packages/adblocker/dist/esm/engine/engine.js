...
export const ENGINE_VERSION = 899;
...
export default class FilterEngine extends EventEmitter {
        ...
        if (networkFilters.length !== 0) {
            ...
            for (const filter of networkFilters) {
                ...
                this.emit('filter-matched', { filter, exception }, {
                    request,
                    filterType: 1 /* FilterType.NETWORK */,
                });
    ...
    getCSPDirectives(request) {
        ...
        for (const filter of matches) {
            if (filter.isException()) {
                if (filter.csp === undefined) {
                    ...
                    this.emit('filter-matched', { exception: filter }, { request, filterType: 1 /* FilterType.NETWORK */ });
                    return undefined;
                }
        ...
        for (const filter of cspFilters.values()) {
            ...
            this.emit('filter-matched', { filter, exception }, { request, filterType: 1 /* FilterType.NETWORK */ });
        }
    ...
    match(request, withMetadata = false) {
                    ...
                    if (searchParamSeparatorIndex !== -1 &&
                        ...
                        for (const [key, filter] of removeparamFilters) {
                            ...
                            if (key === '') {
                                ...
                                this.emit('filter-matched', { filter, exception: removeparamIgnoreFilter }, { request, filterType: 1 /* FilterType.NETWORK */ });
                                break;
                            }
                            if (!key.startsWith('~') &&
        ...
        if (result.filter) {
            this.emit('filter-matched', { filter: result.filter, exception: result.exception }, { request, filterType: 1 /* FilterType.NETWORK */ });
        }
